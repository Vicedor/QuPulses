...
file = rf'ramsay_with_{n}_photons.pkl'
file_classic = rf'classical_ramsay_with_{n}_photons.pkl'
...
def main():
    ...
    if os.path.exists(file):
        ...
        for i, D in enumerate(Deltas):
            ...
            populations[i, :] = result.expect[2]
            intensity1_early[i] = trapezoid(result.expect[5][:cut], times[:cut])
            intensity1_late[i] = trapezoid(result.expect[5][cut:], times[cut:])
            intensity2_early[i] = trapezoid(result.expect[6][:cut], times[:cut])
            intensity2_late[i] = trapezoid(result.expect[6][cut:], times[cut:])
            sys.stdout.flush()
    ...
    plt.plot(extended_deltas, np.concatenate([np.flip(populations[:, t_max2_ind]), populations[:, t_max2_ind]]), '-', label='Fock state')
    if os.path.exists(file_classic):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'atomic_excitation_{n}_photons_with_classical.pdf', format='pdf', dpi=300)
    plt.show()
    ...
    plt.ylim()
    plt.xlabel(r'Detuning $\Delta$')
    plt.ylabel(r'$\int_0^t d\tau \langle \hat{L}_1^\dagger(\tau) \hat{L}_1(\tau) \rangle$')
    plt.tight_layout()
    plt.savefig(f'intensity_{n}_photons.pdf', format='pdf', dpi=300)
    plt.show()
...
def interaction_picture_solution(delta: float = 0) -> qt.solver.Result:
    ...
    theta1 = lambda t: np.arcsin(np.sqrt(f1(t)))
    theta2 = lambda t: np.arcsin(np.sqrt(f2(t)))

    phi_pulse = Pulse(gauss, in_going=True, args=[tp + t_delay + T2, tau])

    g_phi = lambda t: 0 if t < T2 else phi_pulse.g(t)

    sigma_minus: qt.Qobj = qt.tensor(qt.qeye(N), qt.qeye(M), qt.destroy(d), qt.qeye(M), qt.qeye(M))
    sigma_plus: qt.Qobj = sigma_minus.dag()

    au: qt.Qobj = qt.tensor(qt.destroy(N, offset=offset), qt.qeye(M), qt.qeye(d), qt.qeye(M), qt.qeye(M))
    b2: qt.Qobj = qt.tensor(qt.qeye(N), qt.destroy(M), qt.qeye(d), qt.qeye(M), qt.qeye(M))

    c1: qt.Qobj = (au + b2) / np.sqrt(2)
    c2: qt.Qobj = (au - b2) / np.sqrt(2)

    d1: qt.Qobj = qt.tensor(qt.qeye(N), qt.qeye(M), qt.qeye(d), qt.destroy(M), qt.qeye(M))
    d2: qt.Qobj = qt.tensor(qt.qeye(N), qt.qeye(M), qt.qeye(d), qt.qeye(M), qt.destroy(M))

    psi0: qt.Qobj = qt.tensor(qt.basis(N, n, offset=offset), qt.basis(M, 0), qt.basis(d, 0), qt.basis(M, 0), qt.basis(M, 0))

    L1dagL1 = lambda t, state: qt.expect(L1(t).dag() * L1(t), state)
    L2dagL2 = lambda t, state: qt.expect(L2(t).dag() * L2(t), state)
    e_ops = [au.dag() * au, b2.dag() * b2, sigma_plus * sigma_minus, d1.dag() * d1, d2.dag() * d2, L1dagL1, L2dagL2]

    H0 = delta * sigma_plus * sigma_minus
    H1 = lambda t: 1j * np.sqrt(gamma / 2) * w1_pulse.u(t) * (c1.dag() * sigma_minus + cot(2*theta1(t)) * d1.dag() * sigma_minus) + 0.5j * g_phi(t) * np.sqrt(gamma / 2) * sigma_plus * c1
    H2 = lambda t: 1j * np.sqrt(gamma / 2) * w2_pulse.u(t) * (c2.dag() * sigma_minus + cot(2*theta2(t)) * d2.dag() * sigma_minus) + 0.5j * g_phi(t) * np.sqrt(gamma / 2) * sigma_plus * c2

    H = lambda t: H0 + H1(t) + H1(t).dag() + H2(t) + H2(t).dag()

    L1 = lambda t: - np.sqrt(2) * (w1_pulse.u(t) * csc(2*theta1(t)) * d1 + w2_pulse.u(t) * csc(2*theta2(t)) * d2 - 0.5 * g_phi(t) * (c1 + c2)) + np.sqrt(gamma) * sigma_minus
    L2 = lambda t:   np.sqrt(2) * (w1_pulse.u(t) * csc(2*theta1(t)) * d1 - w2_pulse.u(t) * csc(2*theta2(t)) * d2 + 0.5 * g_phi(t) * (c2 - c1))
