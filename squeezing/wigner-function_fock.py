...
def get_v1_and_v2():
    ...
    hu = lambda omega: (gu(omega) - gufu * fu(omega)) / np.sqrt(1 - gufu * fugu)
    ...
    #v1 = lambda omega: (vecs[0][0] - vecs[0][1] * gufu / np.sqrt(1 - fugu * gufu)) * fu(omega) + vecs[0][1] * gu(omega) / np.sqrt(1 - fugu * gufu)
    #v2 = lambda omega: (vecs[1][0] - vecs[1][1] * gufu / np.sqrt(1 - fugu * gufu)) * fu(omega) + vecs[1][1] * gu(omega) / np.sqrt(1 - fugu * gufu)

    v1 = lambda omega: vecs[0][0] * fu(omega) + vecs[1][0] * hu(omega)
    v2 = lambda omega: vecs[0][1] * fu(omega) + vecs[1][1] * hu(omega)
